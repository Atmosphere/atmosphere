...
Real-time AI assistant with **tool calling**, **token budgets**, and **response caching** — powered by
...
- **Token Budget Manager** — tracks per-user token usage with 10,000 token budget; degrades to `gemini-2.0-flash-lite` at 80% usage
...
- `Tell me about token budgets` — explains the budget system
...
| `AiConfig.java` | Configures token budgets, cache inspector, cost filter |
