...
public class StreamingServiceImpl extends RemoteServiceServlet implements StreamingServiceInternalGWT {
    ...
    public void sendMessage(String queueName, String message) {
        BroadcasterFactory.getDefault().lookup(DefaultBroadcaster.class,"GWT")
                .broadcast(new StreamingServiceBusiness.Event(queueName, message));
    }

    public void unsubscribeFromTopic(String queueName) {
        String clientId = getThreadLocalRequest().getSession().getId();
        getServletContext().log("client " + clientId + " unsubscribed from topic " + queueName);
    }
}
