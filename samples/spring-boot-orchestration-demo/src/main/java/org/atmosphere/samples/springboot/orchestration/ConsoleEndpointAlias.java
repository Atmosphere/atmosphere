...
/**
 * Routes the built-in console to the support agent so users interact
 * with support first and get handed off to billing when needed.
 */
@Component
public class ConsoleEndpointAlias {
    ...
    ConsoleEndpointAlias(AtmosphereFramework framework) {
        framework.getAtmosphereConfig().startupHook(f -> {
            var agentPath = "/atmosphere/agent/support";
            ...
            logger.info("Console path {} routes to support agent", consolePath);
        });
    }
}
