...
const AGENTS: Record<string, { label: string; icon: string; color: string; bg: string; backend: string }> = {
  web_search:       { label: 'Research Agent',  icon: '\uD83D\uDD0D', color: '#f59e0b', bg: 'rgba(245,158,11,0.10)', backend: 'JSoup + DuckDuckGo' },
  analyze_strategy: { label: 'Strategy Agent',  icon: '\uD83C\uDFAF', color: '#10b981', bg: 'rgba(16,185,129,0.10)', backend: 'Spring AI' },
  financial_model:  { label: 'Finance Agent',   icon: '\uD83D\uDCB0', color: '#8b5cf6', bg: 'rgba(139,92,246,0.10)', backend: 'Built-in Computation' },
  write_report:     { label: 'Writer Agent',    icon: '\u270D\uFE0F', color: '#ef4444', bg: 'rgba(239,68,68,0.10)',  backend: 'Google ADK' },
};

const CEO = { label: 'CEO', icon: '\uD83D\uDC54', color: '#3b82f6', backend: 'Gemini 2.5 Flash' };
...
export function App() {
  ...
  const request = useMemo(() => ({
    url: `${window.location.protocol}//${window.location.host}/atmosphere/agent/startup-ceo`,
  ...
  return (
    <ChatLayout
      title="Atmosphere Multi-Agent Team"
      subtitle="5 AI agents collaborating in real-time"
      theme="ai"
    >
      <div style={{
        flex: 1,
