...
All samples inherit their Atmosphere version from the parent POM. The target stack is Java 21, Spring Boot 4.1.0, and Quarkus 3.39.5.
...
| [quarkus-chat](quarkus-chat/) | Quarkus 3.39.5 | JAR | — | — | ✅ |
| [quarkus-ai-chat](quarkus-ai-chat/) | Quarkus 3.39.5 + LangChain4j | JAR | — | — | profile only, no CI lane |
...
**Reading the Native Image column.** `✅` means the sample declares a `native` profile *and* a job in [`.github/workflows/native-image-ci.yml`](../.github/workflows/native-image-ci.yml) builds the binary, starts it, opens a long-polling connection to the `@ManagedService` endpoint, asserts the annotated `@Ready` method ran, and then drives real clients against it via `scripts/native/NativeTransportProbe.java` — WebSocket fan-out between two connections (exercising the `@Message` codec round-trip), SSE delivery, and on `spring-boot-chat` the room protocol's join/presence/broadcast. A third job builds [spring-boot-ai-chat](spring-boot-ai-chat/) natively and asserts a real `@AiEndpoint` agent turn returns assistant content. `profile only, no CI lane` means the `native` profile exists but nothing in CI builds or exercises the binary. `—` means no `native` profile is configured.

Still asserted by **no** native lane: transport negotiation and fallback (an atmosphere.js client behaviour — the probe pins each transport explicitly), `@RoomService`-annotated endpoints (the probe drives the room protocol against a programmatic `RoomManager`), history replay on join, `@Disconnect`/`@Heartbeat`, gRPC, and injection beyond what the samples use. Those paths are not known to be broken; they are unproven, and a cell here stays unticked until a lane asserts it.
...
Available `--template` values: `chat`, `one-dep-agent`, `ai-chat`, `ai-tools`, `mcp-server`, `rag`, `agent`, `multi-agent`, `classroom`, `ms-governance`, `coding-agent`, `guarded-agent`, `assistant`, `browser-agent`, `classic-rooms`. See [cli/README.md](../cli/README.md#available-templates) for the template-to-sample mapping.
