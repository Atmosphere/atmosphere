...
public class ChatRoom {
    ...
    private final ConcurrentLinkedQueue<String> users = new ConcurrentLinkedQueue<String>();
    ...
    public RoomsState onReady(final AtmosphereResource r) {
        logger.info("Browser {} connected.", r.uuid());
        if (chatroomName == null) {
            chatroomName = r.getBroadcaster().getID();
        }
        return new RoomsState(r.getAtmosphereConfig().getBroadcasterFactory());
    }
     ...
     * @throws IOException
     */
    @org.atmosphere.config.service.Message(encoders = {JacksonEncoder.class}, decoders = {MessageDecoder.class})
