...
/**
 * Spring Boot 3 parity for the {@code atmosphere.ai.code.*} →
 * {@code org.atmosphere.ai.code.*} system-property bridge: set Spring
 * properties are bridged, a system property the operator already set on the
 * JVM is never overridden (JVM wins), unset properties bridge nothing
 * (default-deny survives), and on context shutdown only the bridge-owned keys
 * are cleared (Ownership, Invariant #1). Mirrors the SB4 starter's
 * {@code CodeExecPropertyBridgeTest}.
 */
