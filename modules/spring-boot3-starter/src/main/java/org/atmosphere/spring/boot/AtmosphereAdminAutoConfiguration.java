...
public class AtmosphereAdminAutoConfiguration {
    ...
    static class AdminMcpBridgeConfiguration {
        ...
        AdminMcpBridge atmosphereAdminMcpBridge(
            ...
            // information-only. Write tools (scale, kill, reload, read_tape)
            // default to REQUIRE_PRINCIPAL so the admin MCP surface isn't a
            // no-auth mutation/pre-redaction-content channel (Correctness
            // Invariant #6 — default deny). Applications that expose admin
            // writes intentionally can inject their own ControlAuthorizer bean.
            ...
            if (Boolean.TRUE.toString().equalsIgnoreCase(
                ...
                // mounting write tools so ALLOW_ALL never reaches a mutation
                // surface (or the pre-redaction read_tape tool).
    ...
    static class CoordinatorAdminConfiguration {
        ...
        CoordinatorController atmosphereAdminCoordinatorController(AtmosphereAdmin admin,
            ...
            // of bean-vs-framework startup ordering (parity with the Spring
            // Boot 4 starter; the previous empty-map wiring always returned []).
