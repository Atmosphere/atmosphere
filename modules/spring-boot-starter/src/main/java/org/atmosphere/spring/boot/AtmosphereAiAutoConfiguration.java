...
public class AtmosphereAiAutoConfiguration {
     ...
     * unauthenticated caller collapses into.
     *
     * <p>Each dimension is overridable —
     * {@code atmosphere.ai.gateway.max-requests-per-window},
     * {@code .window-seconds}, {@code .anonymous-max-requests} — and a
     * {@code CredentialResolver} / {@code GatewayTraceExporter} bean is
     * composed in when present, so the profile is a starting point rather
     * than a ceiling on configurability.</p>
     */
    @Bean
     ...
     * stale gateway behind (Ownership, Correctness Invariant #1 — the
     * installer resets only because it is what installed).
     */
     ...
     * startup, restoring the no-ops on shutdown so the dispatch decorator
     * chain stays consistent across context restarts.
     */
     ...
     *
     * <p>The journal is resolved as: a user-supplied
     * {@link org.atmosphere.ai.resume.RunJournal} bean, else the bundled
     * crash-durable SQLite journal when the optional {@code atmosphere-checkpoint}
     * module is present, else the in-memory journal with a NOT-crash-durable
     * warning (Correctness Invariant #5 — never advertise crash-durability that
     * the runtime cannot honour). A journal this installer created is closed on
     * shutdown (Ownership, Correctness Invariant #1).</p>
     */
    @Bean
     ...
     * stale state across hot-redeploy and test-context restarts. A journal this
