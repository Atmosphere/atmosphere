...
/**
 * The sandboxed Mozilla Rhino evaluator — the Rhino-touching core behind
 * {@link RhinoEvalEngine}. Kept separate so the {@link EvalEngine} adapter has a
 * Rhino-free signature and loads even when the (optional) Rhino dependency is
 * absent; this class is only referenced — and therefore only class-loaded — once
 * {@link RhinoEvalEngine#isAvailable()} has confirmed Rhino is present.
 *
 * <p><strong>Isolation (Correctness Invariants #4, #6).</strong></p>
 * <ul>
 *   <li><b>No host reach.</b> The scope is built with Rhino's
 *       {@code initSafeStandardObjects()}, which installs the ECMAScript
 *       built-ins (<code>Math</code>, <code>JSON</code>, <code>Array</code>, …)
 *       but <em>none</em> of Rhino's LiveConnect Java bridge (<code>java</code>,
 *       <code>Packages</code>, <code>getClass</code>). A deny-all
 *       {@link ClassShutter} is layered on top so no Java class is resolvable
 *       even via a reflective escape. Rhino JS has no built-in file or network
 *       I/O, so the scope cannot touch the host.</li>
 ...
 *   <li><b>Bounded output.</b> The serialized result is capped.</li>
 * </ul>
 *
 ...
 */
final class RhinoScriptSandbox {

    private final int maxOutputChars;
    private final SandboxContextFactory factory;

    RhinoScriptSandbox(int instructionBudget, long timeoutMillis, int maxOutputChars) {
