 ...
 *
 * <h2>Wiring status — consumed by the opt-in file-backed state seal</h2>
 *
 * {@link AgentStateSealer} uses this primitive to seal and verify every file
 * that {@link org.atmosphere.ai.state.FileSystemAgentState} reads or writes,
 * once an operator enables sealing with
 * {@code -Datmosphere.ai.state.seal.enabled=true}. The control is OFF by
 * default because the file-backed state is deliberately hand-editable
 * ({@code cat}/{@code vim}/{@code git}); when it is on, verification fails
 * closed and the {@link AgentStateReseal} step blesses deliberate operator
 * edits. The wiring is pinned by {@code AgentStateIntegrityWiringPinTest} —
 * unhooking the consumer breaks the build.
 *
 * <p>This class moved here from
 * {@code org.atmosphere.coordinator.commitment.AgentStateIntegrity} so the
 * file-backed state (this module) can consume it without a dependency on the
 * coordinator; the old coordinates remain as a deprecated forwarder.</p>
 *
 * <h2>Design</h2>
 * Content-addressed seal: Ed25519-sign the domain-separated payload.
 * Verifier re-builds the payload and re-verifies. Two pieces:
 * <ul>
 ...
 * or a workspace-relative file path) so a seal for one slot cannot be
 * replayed into another.</p>
 *
 * <p>Sibling of the coordinator's {@code Ed25519CommitmentSigner} — both use
 * the same JDK 21 built-in EdDSA provider; same key-management story. A
 ...
 *
 * <p>Relevant to OWASP Agentic A03 Memory Poisoning — as an <b>opt-in</b>
 * control it belongs in that row's notes, not its evidence list, unless a
 * deployment has actually enabled it.</p>
 */
public final class AgentStateIntegrity {
    ...
    public static AgentStateIntegrity generate() {
        try {
            ...
            return new AgentStateIntegrity(pair, fingerprint(pair.getPublic()));
    ...
    /**
     * SHA-256 fingerprint of the public key's encoded form — stable across
     * JVMs. Same derivation as the coordinator's
     * {@code Ed25519CommitmentSigner.fingerprint}, kept local so this module
