...
**Spring AI Alibaba runtime caveat (downstream-library version pin):** Spring AI Alibaba `1.1.2.0` is compiled against Spring AI `1.1.2`, and `spring-ai-alibaba-graph-core-1.1.2.0` hardcodes references to Spring AI 1.1.2-only types (e.g. `org.springframework.ai.deepseek.DeepSeekAssistantMessage`). Forcing Spring AI 2.0.0-M2 across the classpath therefore fails at `ReactAgent` construction with `NoClassDefFoundError`. Spring Boot 4 itself is unaffected (`RestClientAutoConfiguration` is preserved at the renamed FQN `org.springframework.boot.restclient.autoconfigure.RestClientAutoConfiguration`). The integration is blocked on Spring AI Alibaba publishing a Spring AI 2.x-aligned release of `spring-ai-alibaba-agent-framework`. The contract test (`SpringAiAlibabaRuntimeContractTest`) covers the runtime via a `Testable…` subclass that bypasses the OpenAI bean wiring; no Spring Boot 4 dep-swap path runs end-to-end yet. AgentScope (`atmosphere-agentscope`) does not share this constraint — it builds its OpenAI-compatible client through `AiConfig` directly and was validated end-to-end on Spring Boot 4 against Ollama.
