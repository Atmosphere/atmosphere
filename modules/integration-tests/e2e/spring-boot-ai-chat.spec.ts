...
test.describe('Spring Boot AI Chat', () => {
  ...
  //
  // PromptCacheDemoChat echoes the annotation attribute as
  // "prompt.cache.policy" and exposes a sample-level cache-hit transition
  // via "prompt.cache.hit". First request for a given prompt → miss,
  // second → hit. This proves the annotation attribute is reachable at
  // runtime and exhibits cache-hit behavior without requiring a real LLM.
  test('@AiEndpoint(promptCache) surfaces cache-hit on repeated prompt', async () => {
    ...
    expect(metadataValue(firstFrames, 'prompt.cache.hit')).toBe(false);
    ...
    expect(metadataValue(secondFrames, 'prompt.cache.hit')).toBe(true);
