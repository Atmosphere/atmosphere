...
public interface JournalFormat {
    ...
    JournalFormat STANDARD_LOG = new StandardLog();

    /** Markdown table with Event, Agent, Detail, and Duration columns. */
    JournalFormat MARKDOWN = new Markdown();

    /** Plain-text log format using {@link CoordinationEvent#toLogLine()}. */
    class StandardLog implements JournalFormat {
        @Override
        public String format(List<CoordinationEvent> events) {
    ...
    class Markdown implements JournalFormat {
        @Override
        public String format(List<CoordinationEvent> events) {
            var sb = new StringBuilder();
