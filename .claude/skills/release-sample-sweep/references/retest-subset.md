...
| Container | Samples |
|---|---|
| Spring Boot | the 27 `spring-boot-*` samples (recount: `find samples -maxdepth 1 -type d -name 'spring-boot-*' | wc -l`) |
...
| Console bundle (`modules/spring-boot-starter/frontend/`) or `atmosphere.js` | One sample **per Console surface class**: `spring-boot-chat` (chat), `guarded-email-agent` (Validation), `checkpoint-agent` (Checkpoints), `coding-agent` (Interactions), `multi-agent-startup-team` (Tape + fleet), `admin-bundle` (admin), plus `rag-chat` / `agui-chat` / `grpc-chat` for the non-WS transports | The Console is the validation surface for 24 samples (`git grep -l atmosphere/console -- samples/ | cut -d/ -f2 | sort -u | wc -l` — `git grep`, not `grep -r`: a plain recursive grep also counts the Console bundle copied into `target/` by a previous build, which inflates the figure on a dirty working copy); a bundle change invalidates all of their UI evidence |
...
| `modules/spring-boot-starter` (Java) | `ai-chat`, `spring-boot-chat`, `one-dep-agent`, `admin-bundle` | Auto-configuration reaches all 27 Spring Boot samples; these four cover AI, plain, single-dep, and bundle wiring |
