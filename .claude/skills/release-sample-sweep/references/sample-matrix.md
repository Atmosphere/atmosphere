...
| 13 | `spring-boot-personal-assistant` | 9113 | spring-boot | `/atmosphere/agent/primary-assistant` | Console | Cross-session fact recall (`LongTermMemory`): state a fact with `?user=alice`, **disconnect**, reconnect as `?user=alice` and require the fact back — then reconnect as `?user=bob` and require it absent. Same-session recall proves only the model's context window, and a same-connection check passed for weeks while extraction was dropping every fact. Identity comes from `PrimaryAssistant.@Ready` reading `?user=` (default `demo-user`); a blank `ai.userId` silently disables both recall and extraction. |
...
| 25 | `spring-boot-ai-classroom` | 9125 | spring-boot | `/atmosphere/classroom/general` | Console (`· webtransport`) | Room join over WebTransport (badge must read `· webtransport`, no ` · fallback`). Open `/atmosphere/classroom/math` and `/atmosphere/classroom/code` as **separate pages**: a prompt in one must stream to every client in **that** room and to **none** in the other. Fan-out alone is not a pass — it passes *harder* when every room collapses onto one empty-id broadcaster, which is exactly the bug 945ff83d1b fixed. This is the only row driving a templated `@AiEndpoint` path over WebTransport. |
...
| 32 | `spring-boot-team-rooms` | 9131 | spring-boot | `/atmosphere/console/` | Console | Classic-annotation stack with no AI on the classpath. Point the Console at `/atmosphere/rooms/<name>`; two different room names must show independent membership and history. `GET /api/presence` reports per-room occupancy (from the `@BroadcasterListenerService`) and replay counters (from the `@BroadcasterCacheListenerService`). Redaction must be asserted with a **raw WS client, not the Console**: (a) `/api/console/info` reports `mode:"ai"` on a fresh boot, so the Console does not render this endpoint as a room, and (b) a client-side mask is indistinguishable from a server-side one in the DOM. Connect a WS client to `/atmosphere/rooms/general`, post a bearer-token-shaped string, and require the **received frame** to contain `[redacted]` — the encoded payload on the wire. A filter that only matches the domain type never fires on the managed path (the encoder runs first and hands the filter a `RawMessage`), which is exactly how this shipped broken. |
| 33 | `spring-boot-low-level-handlers` | 9132 | spring-boot | `/atmosphere/console/` | Console | Two feeds one layer apart: `/atmosphere/raw/ops` (raw `AtmosphereHandler`) and `/atmosphere/managed/ops` (`@ManagedService` twin). `GET /api/health` must show all three listener layers **and the counters must move under drive**: read it before and after connecting, and require `resource.suspended` to rise once per connection (and `resource.disconnected` / `transport.closes` on close). Structure alone proves nothing — the endpoint returns all three keys with zero connections even when a listener is not installed at all, which is how a listener implementing the wrong interface passed this row. `@RoomAuth` is on the raw handler only; it cannot resolve on the managed twin |
