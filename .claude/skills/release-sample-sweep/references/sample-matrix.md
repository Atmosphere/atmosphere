...
| 26 | `spring-boot-coding-agent` | 9126 | spring-boot | `/actuator/health` | Console + `/api/interactions` | Durable-step timeline: a background run reaches COMPLETED with its step + metadata readable. Needs Docker for the sandbox; if the model answers as text rather than invoking the sandbox tool, say so — do not claim a container ran |
...
| `spring-boot-coding-agent` | Docker for the sandbox | Interactions timeline verified; sandbox run only if the model invokes the tool |
