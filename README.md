...
| [`atmosphere-embabel`](https://atmosphere.github.io/docs/integrations/embabel/) | Embabel 1.5.0 | 4.0 | agent orchestration, tool calling, vision, conversation memory | Embabel 1.5.0 is built on Spring Boot 4.1 / Spring AI 2.0, so this adapter now runs on the default Spring Boot 4 build. Spring Boot 3.5 is no longer supported here — Embabel's 1.0.x line targets Spring Boot 3.5, but this adapter is not built or tested against it. |
...
**Requirements:** Java 21+ · Spring Boot 4.1.0 or Spring Boot 3.5 via `-Pspring-boot3` · Quarkus 3.36.3+ · current release from the Maven Central badge above.
