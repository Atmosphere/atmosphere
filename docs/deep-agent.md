...
| Long-term memory | `MEMORY` | Attaches a `LongTermMemoryInterceptor`: facts extracted at session close, recalled into the system prompt on the next connection. Store resolution: a bridged `LongTermMemory` bean → a `ServiceLoader` `LongTermMemoryProvider` → the zero-dep `InMemoryLongTermMemory` fallback. **Requires a caller identity**: facts are keyed by the `ai.userId` request attribute, which `AiEndpointHandler` defaults from the servlet `Principal`. With no auth configured the attribute stays unset and *both* recall and on-close extraction short-circuit (DEBUG log only) — the harness still reports the primitive ACTIVE, because the store resolved. Set `ai.userId` yourself in `@Ready` or from an `AtmosphereInterceptor`; see [`samples/spring-boot-personal-assistant`](../samples/spring-boot-personal-assistant/README.md) for a worked example. |
